...
The analysis only asks for a tidy frame of adjusted close prices (dates on the
index, tickers on the columns). Swapping yfinance for a paid feed is a one-class
change. Downloads are cached to parquet, keyed by the request.
"""
...
class YFinanceLoader:
    ...
    def _download(
        ...
        # Keep column order matching the requested tickers where possible.
        ordered = [t for t in tickers if t in prices.columns]
        return prices[ordered]


def daily_returns(prices: pd.DataFrame) -> pd.DataFrame:
    """Simple daily returns, with the first all-NaN row dropped."""
